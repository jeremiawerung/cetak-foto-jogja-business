@extends('layouts.internal')

@section('title', 'Kelola Booking Studio - Internal')

@section('content')
@include('admin.partials.header')

<main class="max-w-5xl mx-auto px-4 sm:px-6 py-10">
    <h1 class="text-xl font-bold text-neutral-900">Kelola Booking Studio</h1>
    <p class="mt-1 text-sm text-neutral-500">Booking sewa fotografer yang masuk dari halaman /sewa-fotografer.</p>

    @if (session('status'))
        <div class="mt-4 rounded-lg bg-emerald-50 border border-emerald-100 px-4 py-3 text-sm text-emerald-700">
            {{ session('status') }}
        </div>
    @endif

    <div class="mt-6 flex flex-wrap items-center gap-2 text-sm">
        <a href="{{ route('admin.booking-studio.index') }}"
           class="rounded-full border px-3 py-1 {{ ! $statusTerpilih ? 'border-rose-300 bg-rose-50 text-rose-700 font-semibold' : 'border-neutral-200 text-neutral-500 hover:border-rose-300' }}">
            Semua
        </a>
        @foreach (\App\Models\PhotographerBooking::STATUSES as $value => $label)
            <a href="{{ route('admin.booking-studio.index', ['status' => $value]) }}"
               class="rounded-full border px-3 py-1 {{ $statusTerpilih === $value ? 'border-rose-300 bg-rose-50 text-rose-700 font-semibold' : 'border-neutral-200 text-neutral-500 hover:border-rose-300' }}">
                {{ $label }}
            </a>
        @endforeach
    </div>

    <div class="mt-4 overflow-x-auto rounded-xl border border-neutral-200">
        <table class="w-full text-sm">
            <thead>
                <tr class="text-left text-neutral-500 bg-neutral-50 border-b border-neutral-200">
                    <th class="py-2 px-3">Tanggal</th>
                    <th class="py-2 px-3">Jam</th>
                    <th class="py-2 px-3">Customer</th>
                    <th class="py-2 px-3">Lokasi</th>
                    <th class="py-2 px-3">Estimasi Orang</th>
                    <th class="py-2 px-3">Status</th>
                    <th class="py-2 px-3"></th>
                </tr>
            </thead>
            <tbody>
                @forelse ($bookings as $booking)
                    <tr class="border-b border-neutral-100 align-top">
                        <td class="py-2 px-3 text-neutral-600 whitespace-nowrap">{{ $booking->tanggal->format('d/m/Y') }}</td>
                        <td class="py-2 px-3 text-neutral-600">{{ substr($booking->jam, 0, 5) }}</td>
                        <td class="py-2 px-3">
                            <div class="font-medium text-neutral-800">{{ $booking->nama }}</div>
                            <div class="text-xs text-neutral-500">{{ $booking->no_hp }}</div>
                        </td>
                        <td class="py-2 px-3 text-neutral-600">{{ $booking->lokasi }}</td>
                        <td class="py-2 px-3 text-neutral-600">{{ $booking->estimasi_orang ?: '-' }}</td>
                        <td class="py-2 px-3">
                            <span class="inline-block rounded-full border border-neutral-200 bg-neutral-50 px-2 py-0.5 text-xs font-medium text-neutral-600">
                                {{ \App\Models\PhotographerBooking::STATUSES[$booking->status] ?? $booking->status }}
                            </span>
                        </td>
                        <td class="py-2 px-3">
                            <a href="{{ route('admin.booking-studio.show', $booking) }}" class="text-rose-600 font-medium hover:underline">Detail &rarr;</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="py-6 px-3 text-center text-neutral-400">Belum ada booking.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    </div>

    <div class="mt-4">
        {{ $bookings->links() }}
    </div>
</main>
@endsection
